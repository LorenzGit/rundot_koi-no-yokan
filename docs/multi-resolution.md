...
Do not use bare `env(safe-area-inset-*)` as the fallback. Desktop WKWebView
cannot reliably rotate those values during device simulation. In landscape,
that commonly leaves the portrait home-indicator inset at the bottom while
reporting zero on the sides: the result is a false bottom gap and controls
under the notch. Keep this chain for every edge:
